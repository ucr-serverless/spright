...
shm_mgr()
{
	...
	echo ${config_file}
	echo ${pci_arg:+-p $pci_arg}
	echo ${file_arg:+-f $file_arg}

	exec ${build_path}/shm_mgr_doca_${io} \
		-l ${CPU_SHM_MGR[0]} \
		--file-prefix=spright \
		--proc-type=primary \
		--no-telemetry \
...
gateway()
{
	...
	echo ${pci_arg:+-p $pci_arg}
	echo ${rep_arg:+-r $rep_arg}
	echo ${file_arg:+-f $file_arg}

	exec ${build_path}/gateway_doca_${io} \
