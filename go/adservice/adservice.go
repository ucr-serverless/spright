...
var (
	...
	nfID uint8
	numWorkers int
	nfName string = "AdService"
	nfNameToIdMap map[string]uint8
	AdService = &server{}
)

var adsMap map[string]*pb.Ad = map[string]*pb.Ad {
	"clothing": &pb.Ad{
...
func getAdsByCategory(contextKey string) *pb.Ad {
	if value, ok := adsMap[contextKey]; ok {
		return value
	} else {
		// Key Not Found
		return nil
...
func getRandomAds() []*pb.Ad {
	...
	for i := 0; i < MAX_ADS_TO_SERVE; i++ {
		k := rand.Intn(len(adsList))
		out = append(out, adsList[k])
	}
	return out
}

func (s *server) GetAds(in *pb.AdRequest) (*pb.AdResponse, error) {
	log.Infof("[GetAds] received ad request (context_words=%v)", in.GetContextKeys())
	defer log.Info("[GetAds] completed request")

	var allAds []*pb.Ad
	if (len(in.GetContextKeys()) > 0) {
		log.Info("Constructing Ads using received context.")
		for _, k := range in.GetContextKeys() {
			ad := getAdsByCategory(k)
			if ad != nil {
...
func (s *server) AdPrint() {
	fmt.Printf("This is Ad Service\n")
}
