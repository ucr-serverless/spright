...
func nfDispatcher(txn *C.struct_http_transaction) C.uint8_t {
	...
	if rpcHandler == "ChargeHandler" {
		next_nf, next_rpcHandler = ChargeHandler(txn)
	} else {
...
func ChargeHandler(txn *C.struct_http_transaction) (C.uint8_t, string) {
	var next_nf C.uint8_t
	var next_rpcHandler string

	if val, ok := nfNameToIdMap[C.GoString(&txn.caller_nf[0])]; ok {
		next_nf = C.uchar(val)
		next_rpcHandler = "ChargeResponseHandler"
	} else {
...
func (s *server) Charge(in *pb.ChargeRequest) (*pb.ChargeResponse, error) {
	log.Info("[Charge] received request")
	defer log.Info("[Charge] completed request")

	amount := in.GetAmount()
	cardNumber := in.GetCreditCard().GetCreditCardNumber()

	cardType := ""
	valid := false
	card := creditcard.Card{
		Number: cardNumber,
		Cvv: strconv.Itoa(int(in.GetCreditCard().GetCreditCardCvv())),
		Month: strconv.Itoa(int(in.GetCreditCard().GetCreditCardExpirationMonth())),
		Year: strconv.Itoa(int(in.GetCreditCard().GetCreditCardExpirationYear())),
	}
	if card.Method() == nil {
		cardType = card.Company.Short
	}
	if card.Validate(true) == nil { // Allow test card number - https://stripe.com/docs/testing
		valid = true
	}

	if (!valid) { // throw InvalidCreditCard 
		err_msg := fmt.Sprintf("Credit card info is invalid")
		log.Warnf(err_msg)
	...
	if (!(cardType == "visa" || cardType == "mastercard")) { // throw UnacceptedCreditCard 
		err_msg := fmt.Sprintf("Sorry, we cannot process %v credit cards. Only VISA or MasterCard is accepted.", cardType)
		log.Warnf(err_msg)
		return &pb.ChargeResponse{
			TransactionId: "",
		}, errors.New(err_msg)
	...
	year := in.GetCreditCard().GetCreditCardExpirationYear()
	month := in.GetCreditCard().GetCreditCardExpirationMonth()

	if ((int32(currentYear) * 12 + int32(currentMonth)) > (year * 12 + month)) { // throw ExpiredCreditCard
		err_msg := fmt.Sprintf("Your credit card (ending %v) expired on %v/%v", cardNumber[len(cardNumber)-4:len(cardNumber)], month, year)
		log.Warnf(err_msg)
		return &pb.ChargeResponse{
			TransactionId: "",
		}, errors.New(err_msg)
	...
	log.Infof("Transaction processed: %v ending %v Amount: %v%v.%v", cardType, cardNumber[len(cardNumber)-4:len(cardNumber)], amount.GetCurrencyCode(), amount.GetUnits(), amount.GetNanos())
