...
int dispatcher(void *arg)
{
    ...
    while (1)
    {
        ...
        for (int i = 0; i < nfds; i++)
        {
            ...
            // TODO: change to DOCA Comm Channel
...
static int http_response(struct http_transaction *txn)
{
    int sockfd = txn->sockfd;

    txn->length_response = strlen(HTTP_RESPONSE);
    memcpy(txn->response, HTTP_RESPONSE, txn->length_response);

    /* TODO: Handle incomplete writes */
    ssize_t bytes_sent = write(sockfd, txn->response, txn->length_response);
    if (unlikely(bytes_sent == -1))
    {
        log_error("write() error: %s", strerror(errno));
        return -1;
...
static int conn_read(int sockfd)
{
    ...
    // TODO: use exported buffers from host
    ret = rte_mempool_get(cfg->mempool, (void **)&txn);
    if (unlikely(ret < 0))
    {
        log_error("rte_mempool_get() error: %s", rte_strerror(-ret));
...
static int server_init(struct server_vars *sv)
{
    ...
    log_info("Initializing DOCA Comm Channel I/O...");
    // ret = io_init();
    // if (unlikely(ret == -1))
    // {
...
static int gateway(struct dma_copy_cfg *dma_cfg,
...
int main(int argc, char **argv)
{
    ...
    if (unlikely(argc == 1))
    {
        log_error("DOCA params not provided");
    ...
    ret = gateway(&dma_cfg, ep, &peer_addr);
    if (unlikely(ret == -1))
    {
